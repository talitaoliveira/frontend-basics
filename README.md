# FrontEnd Basics
Neste repositório está todo o conteúdo visto duarnte o curso. O conteúdo aqui será atualizado com o passar das aulas.


* [1. Iniciando](#1-iniciando)
* [2. Conteúdo do módulo de FrontEnd](#2-conteúdo-do-módulo-de-frontend)
* [3. Primeiros passos](#2-primeiros-passos)
* [4. Layout do site a ser desenvolvido](#4-layout-do-site-a-ser-desenvolvido)


## 1. Iniciando

### 1.1 Onde escrever nosso código:

- 🌟[VSCode](https://code.visualstudio.com/download)
- [Sublime Text](https://www.sublimetext.com/3)

### 1.2 Onde visualizar o resultado do código:

- 🌟[Google Chrome](https://www.google.com/intl/pt-BR/chrome/)
- [Firefox](https://www.mozilla.org/pt-BR/firefox/new/)

## 2. Conteúdo do módulo de FrontEnd

- [Aula 01 - HTML - Elementos, Tags, Estrutura, Principais elementos](Aula01/)
- [Aula 02 - HTML - Tabelas, Iframe, Ancoras/Marcadores, Audio/Video](Aula02/)
- [Aula 03 - HTML - Formulário, Elementos de Form, Tipos de Input](Aula03/)
- [Aula 04 - CSS - Conteito CSS, Uso no HTML (interno/externo/inline), Seletores, Fontes](Aula03/)

...

## 3. Primeiros passos

### 3.1 HTML
- No editor de texto abra um novo arquivo e salve (Ctrl+S) com o nome de sua preferencia

Regras para o nome do arquivo:
- **Não deve conter espaços**
- Deve ter a extensão `.html`
<br>exemplos: `index.html`, `meuarquivo.html`, `meuArquivo.html`
- Pode usar o [arquivo de exemplo](Aula01/exemplo-documento.html) com o template inicial para começar uma página HTML
- Sempre que alterar o arquivo não esqueça de **salvar**
- Saiba onde seu arquivo está para que consiga dar dois cliques nele e que abra no navegador

### 3.2 CSS
- No editor de texto abra um novo arquivo e salve (Ctrl+S) com o nome de sua preferencia

Regras para o nome do arquivo:
- **Não deve conter espaços**
- Deve ter a extensão `.css`
<br>exemplos: `style.css`, `estilo.css`

Tendo os dois documentos HTML e CSS é preciso fazer a "ligação" do CSS com o HTML, para que o seu site passe a enxergar os estilos:
- No seu HTML dentro da tag `<head>`, após a tag `<title>` insira a tag `<link>` referenciando o seu arquivo de css

```
    <link rel="stylesheet" href="estilo.css">
```

## 4. Layout do site a ser desenvolvido

<img src="Layout/Layout%20Site%20Pessoal.png" width="50%">

